...
func (s *Item) MarshalBSON() ([]byte, error) {
	var doc = bson.M{"tid": s.tid, "num": s.num}
	return bson.Marshal(doc)
}
func (s *Item) UnmarshalBSON(data []byte) error {
	doc := struct {
		Tid int32 `bson:"tid"`
		Num int32 `bson:"num"`
	}{}
...
func (s *Item) BuildBson(m bson.M, preKey string) {
	dirty := s.GetDirty()
	if dirty == 0 {
		return
	}
...
func (s *BItem) BuildBson(m bson.M, preKey string) {
	dirty := s.GetDirty()
	if dirty == 0 {
		return
	}
